...
const getIcon = (role: string) => {
  ...
  if (r.includes('vfx') || r.includes('animation')) return <Sparkles className="w-5 h-5 text-purple-400" />;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isCEO ? "border-cyan-500/80 shadow-[0_0_30px_rgba(6,182,212,0.3)] w-[100px]" : "border-white/10 hover:border-white/20 shadow-xl w-[90px]",
    ...
    >
      ...
      >
        ...
        <h3 className={cn(
          "text-[8px] font-bold text-white/90 truncate",
          isCEO && "text-cyan-400"
        )}>{name}</h3>
        <div className="flex items-center justify-between mt-0.5">
          <p className="text-[5px] text-white/40 font-mono uppercase tracking-[0.1em]">{role}</p>
